...
- [x] Configure `"use cache"`, `cacheLife`, `cacheTag`, `updateTag`, and `revalidatePath` helpers/tests without deprecated single-argument `revalidateTag` or `unstable_*` APIs.
...
- [x] Build create/edit category dialog and archive/restore flow with validation/conflict/component tests.
...
- [x] Implement stock movement list queries with product/type/actor/date/order filters and integration tests.
...
- [x] Implement `updateProduct` without stock mutation, including SKU conflicts, inactive-category handling, audit diff, and tests.
...
- [x] Add idempotent demo category/product/opening-movement seed data through the completed inventory services.
...
- [x] Build Invoice register TanStack grid with status/customer/date filters, void state, links, and download action.
- [x] Build server-rendered Invoice detail page with snapshot lines, bill-to data, totals, order link, and VOID treatment.
- [x] Build shared invoice document view model and `@react-pdf/renderer` document with deterministic content tests.
- [x] Implement authenticated `GET /api/invoices/[invoiceId]/pdf` streaming handler with safe filename/headers/statuses and tests.
- [x] Add tests proving Inventory and unauthenticated users cannot read invoice pages/data or download PDFs.
